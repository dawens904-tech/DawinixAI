...
// Always use service role — no user JWT needed for admin operations
...
async function sendWhatsAppText(to: string, body: string) {
  if (!PHONE_NUMBER_ID || !ACCESS_TOKEN) {
    throw new Error("PHONE_NUMBER_ID or ACCESS_TOKEN secret not configured in Edge Function secrets");
  }
...
async function updateBotProfile(name?: string, about?: string) {
  ...
  if (!profileRes.ok) {
    const errMsg = profileData?.error?.message ?? `WhatsApp API HTTP ${profileRes.status}`;
    throw new Error(errMsg);
  }
  return profileData;
}
...
serve(async (req) => {
    ...
    try {
      ...
      if (body.profile_name) {
        await supabase.from("bot_config").upsert(
          { key: "bot_name", value: JSON.stringify(body.profile_name), updated_at: new Date().toISOString() },
          { onConflict: "key" }
        );
      }
      if (body.profile_about) {
        await supabase.from("bot_config").upsert(
          { key: "bot_about", value: JSON.stringify(body.profile_about), updated_at: new Date().toISOString() },
          { onConflict: "key" }
        );
      }
  ...
  return new Response(JSON.stringify({ error: "Unknown action. Use 'send' or 'update_profile'" }), {
    status: 400,
    headers: { ...corsHeaders, "Content-Type": "application/json" },
  });
});
